...
void CColladaAnimation::Freeeezzzze( const xmlChar *animName )
{
	CColladaAnimation *animation = CColladaAnimation::Get( animName );
	std::vector<float> &anim = const_cast<std::vector<float> &> ( animation->matrixAnim->GetFloatArray() );

	for( size_t i = 16; i < anim.size(); i += 16 )
	{
